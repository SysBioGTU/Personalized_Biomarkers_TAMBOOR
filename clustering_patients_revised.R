...
#combine each prediction matrix into a single matrix for SN datasets
...
#filtered to exclude features/metabolites with more than 90% of 
#predictions being zero across 104 patients
# Count zeros in each row
zero_counts <- rowSums(xyz == 0)
# Filter rows 
filtered_data <- xyz[zero_counts <= 93, ]
...
colnames(d)<-paste("n",seq(1:665), sep = "_")
...
source <- c(rep("SN_data", 104), rep("LB_data", 81))
